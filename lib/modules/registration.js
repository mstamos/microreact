var AuthenticationModule = (function () {
    ...
    this.notEmptyFields = function (registerData) {
        if (!registerData.username) {
            throw new Meteor.Error(400, TAPi18n.__('username_not_empty'));
        } else if (!registerData.email) {
            throw new Meteor.Error(400, TAPi18n.__('email_not_empty'));
        } else if (!registerData.password) {
            throw new Meteor.Error(400, TAPi18n.__('password_not_empty'));
        } else if (!registerData.profile.userType) {
            throw new Meteor.Error(400, TAPi18n.__('userType_not_empty'));
        }
    }
    ...
    this.isValidPassword = function (password) {
        if (password.length < 6) {
            ...
            throw new Meteor.Error(406, TAPi18n.__('password_six_char'));
            return false;
        }
        return true;
    }
    ...
    this.checkFields = function (registerData) {
        ...
        if (!this.isEmail(registerData.email)) {
            throw new Meteor.Error(406, TAPi18n.__('valid_email_address'));
        }
    };
